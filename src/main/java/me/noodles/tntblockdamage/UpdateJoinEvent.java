...
public class UpdateJoinEvent implements Listener
{
    ...
    public void onJoin(final PlayerJoinEvent e) {
    	...
    	if (p.hasPermission("tntblockdamage.update")) {
    		if (TNTBlockDamage.getPlugin().getConfig().getBoolean("Update.Enabled") == true){
    		this.checker = new UpdateChecker(TNTBlockDamage.plugin);
                        if (this.checker.isConnected()) {
                            if (this.checker.hasUpdate()) {
                                ...
                                p.sendMessage(ChatColor.RED + "Your version: " + TNTBlockDamage.plugin.getDescription().getVersion());
                                p.sendMessage(ChatColor.GRAY + "=========================");
                            }
                        }               
       }
